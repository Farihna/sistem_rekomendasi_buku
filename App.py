...
from utils.data_loader import (
    ...
    load_csv_data,                   # Untuk memuat df_ratings.csv
    load_books,                      # Untuk memuat .csv
    load_recommender_model_as_tfsmlayer, # Untuk memuat model CF sebagai TFSMLayer
)
from utils.cf_utils import get_cf_recommendation
...
isbn_encoding_cf = load_pickle_file("assets/encoders/isbn_encoding.pkl")
df_ratings = load_csv_data("assets/data/df_ratings.csv")
cf_book_titles = load_csv_data("assets/data/clean_books.csv")

# Model CF (TFSMLayer)
cf_model = load_recommender_model_as_tfsmlayer("assets/cf_model/saved_model")

# Aset CBF
df_books_cbf = load_books("assets/data/books_cbf.csv")
...
if df_books_cbf is not None and not df_books_cbf.empty:
    book_titles = df_books_cbf['title'].tolist()
...
book_df = load_books("assets/data/clean_books.csv")
...
# Contoh user ID yang tersedia
user_id_tes = [276744, 276726]

# Jika belum login, tampilkan form login
if not st.session_state.logged_in:
    st.subheader("🔐 Login Pengguna")
    selected_user_id_cf = st.selectbox("Pilih User ID untuk Login:", options=user_id_tes, index=0)
    ...
    if all([user_encoding, isbn_encoding_cf, df_ratings is not None, cf_model, book_df is not None]):
        # Ambil dan tampilkan rekomendasi CF
        rekomendasi_df = get_cf_recommendation(
            user_id,
            cf_model,
            df_ratings,
            book_df,
